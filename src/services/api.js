...
// DCC 스트리밍 API
export const streamDCCAPI = async (choice, message, chatroomId, onData) => {
  console.log('🚀 Sending DCC request:', { choice, message, chatroomId })
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/dcc_stream`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({
        choice: choice,
    ...
    console.log('📡 DCC Response status:', response.status, response.statusText)
      ...
      for (const line of lines) {
        if (line.trim() && line.startsWith('data: ')) {
          try {
            ...
            if (jsonString) {
              const data = JSON.parse(jsonString)
              console.log('✅ Successfully parsed DCC streaming data:', Object.keys(data))
              onData(data)
            }
          } catch (e) {
            console.error('❌ Error parsing DCC streaming data:', e)
            console.error('❌ Problematic line:', line.substring(0, 200) + '...')
          }
        }
      }
    ...
    if (buffer.trim() && buffer.startsWith('data: ')) {
      try {
        ...
        if (jsonString) {
          const data = JSON.parse(jsonString)
          console.log('✅ Successfully parsed final DCC streaming data:', Object.keys(data))
          onData(data)
        }
      } catch (e) {
        console.error('❌ Error parsing final DCC streaming data:', e)
      }
    }
  } catch (error) {
    console.error('Error in streamDCCAPI:', error)
    throw error
  }
}
