...
**산점도를 생성하면 회귀선이 자동으로 추가됩니다!**

```javascript
// 사용자가 단순히 산점도만 요청해도
"온도와 수율의 산점도 그려줘"

// 백엔드에서 reference_lines 필드를 보내지 않거나, null, 빈 문자열("")을 보내면
→ 산점도 점들 표시
→ 파란색 실선 회귀선 자동 추가
→ 범례에 "회귀선" 표시
```
...
**결과:**
- 각 데이터 포인트가 점으로 표시
- ⭐ 파란색 실선 회귀선이 **자동으로** 추가됨
- 온도 증가에 따른 수율 변화 경향 명확히 파악

### 예시 2: 추가 참조선과 함께 (회귀선 + 평균 + 목표)
